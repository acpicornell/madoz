...
- `data.json` (~770 KB) — 1214 entrades amb tot el text + URL del scrape (generat amb `scripts/export_web_data.py`)

## Pestanyes

1. **Explorar** — filtres (cerca, illa, partit, municipi, tipus, volum, confiança, només-amb-scrape), taula sortable, descàrrega CSV dels resultats actuals. Clica una fila per veure la descripció completa, els estadístics (vecinos, almas, contribución...), les referències creuades i l'enllaç a diccionariomadoz.com.
2. **Estadístiques** — recomptes per illa, tipus de lloc, partit judicial, top 20 municipis, cobertura per volum i connexió amb el scrape.
